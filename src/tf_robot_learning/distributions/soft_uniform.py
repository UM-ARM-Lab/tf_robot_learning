    ...
    def log_prob(self, value, name="log_prob"):
        ...
        if self._squared:
            if self._high.shape.ndims == 0:
                ...
                z = tf.reduce_prod(z)
                log_unnormalized_prob += tf.reduce_sum((self._temp * self._activation(-value + self._low[None])) ** 2,
                                                       -1)
                log_unnormalized_prob += tf.reduce_sum((self._temp * self._activation(value - self._high[None])) ** 2,
                                                       -1)
        else:
            if self._high.shape.ndims == 0:
